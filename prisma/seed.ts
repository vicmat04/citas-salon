...
async function main() {
	...
	if (!adminEmail || !adminPassword || !supabaseUrl || !supabaseServiceKey) {
		console.log(
			"Skipping admin and demo salon creation: missing env vars (SEED_ADMIN_EMAIL, SEED_ADMIN_PASSWORD, NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY).",
		);
		return;
	}
	...
	if (existingAuthAdmin) {
		...
		const { data: newAuthAdmin, error: createError } =
