...
vi.mock("@/lib/salons/lifecycle", async (importOriginal) => {
	const original =
		await importOriginal<typeof import("@/lib/salons/lifecycle")>();
	return {
		...original,
...
describe("/s/[slug]/inactive", () => {
	...
	it.each(["suspended", "cancelled"])(
		"renders a non-sensitive %s notice and support action",
		async (status) => {
	...
	it.each([null, salon("pending")])(
		"not-founds unknown and pending salons",
		async (value) => {
	...
	it.each(["trial", "active"])(
		"redirects an operational %s salon to its public page",
		async (status) => {
			mocks.findUnique.mockResolvedValue(salon(status));
			await expect(InactiveSalonPage({ params })).rejects.toThrow(
				"REDIRECT:/acme",
			);
		},
	);
...
describe("public salon and booking route guards", () => {
	...
	it.each([
		...
		"guards the %s page before rendering public mock content",
		async (_name, Page) => {
	...
	it.each([
		...
		"terminates the %s page when a fresh guard observes suspension",
		async (_name, Page) => {
