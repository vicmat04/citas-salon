...
export default async function SalonLayout({
	...
	if (salon.status === "trial") {
		try {
			...
			remainingTrialDays = getTrialBannerDays(
				salon.status,
				endDate,
				salon.timezone,
			);
			if (remainingTrialDays === null) {
				console.warn(
					"Trial subscription end date unavailable for a verified trial salon",
				);
			}
		} catch {
			console.warn(
				"Trial subscription lookup unavailable for a verified trial salon",
			);
		}
	}
