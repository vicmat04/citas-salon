...
vi.mock('@/lib/auth/helpers', () => ({ requireSalonOwner: mocks.requireSalonOwner }))
vi.mock('@/lib/db', () => ({
  default: {
...
import { createSpecialist, deleteSpecialist, updateSalonSettings } from './owner'
...
describe('owner actions', () => {
  beforeEach(() => {
    mocks.requireSalonOwner.mockResolvedValue({ salon: { id: 'verified-salon' } })
  ...
  it('updates only the salon freshly verified from the slug', async () => {
    const result = await updateSalonSettings(
      form({ name: 'Acme', slogan: '', phone: '', address: '', themeColor: '#112233' }),
      'acme',
    )
  ...
  it('scopes specialist deletion to both resource and verified salon IDs', async () => {
    await expect(deleteSpecialist('specialist-from-other-salon', 'acme')).resolves.toEqual({
      success: true,
    })
  ...
  ])('performs no %s write when the fresh lifecycle/owner guard blocks', async (_name, action) => {
