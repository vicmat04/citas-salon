...
const registrationSchema = z.object({
	...
	salonName: z
		.string()
		.trim()
		.min(1, "El nombre del salón es requerido.")
		.max(160),
		...
		if (error instanceof ProvisioningError) {
			if (error.code === "CONFLICT") {
				return {
					...
					code: "CONFLICT",
					message:
						"No fue posible registrar esta cuenta. Intenta iniciar sesión.",
				};
			}
			if (error.code === "CONFIG") {
				return {
					...
					code: "INTERNAL",
					message:
						"Falta la configuración de plan o credenciales en el servidor (variables de entorno).",
				};
			}
		}
